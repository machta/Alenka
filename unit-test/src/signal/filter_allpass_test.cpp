...
template <class T>
void test(function<void(FilterProcessor<T> *)> change,
          function<void(T, T)> compare) {
...

  {
    ...
    cl_mem inBuffer =
        clCreateBuffer(context.getCLContext(), flags | CL_MEM_COPY_HOST_PTR,
                       (n + 2) * sizeof(T), signal.data(), &err);
    checkClErrorCode(err, "clCreateBuffer");

    cl_mem outBuffer = clCreateBuffer(context.getCLContext(), flags,
                                      (n + 2) * sizeof(T), nullptr, &err);
    checkClErrorCode(err, "clCreateBuffer");

    processor.process(inBuffer, outBuffer, queue);

    err = clEnqueueReadBuffer(queue, outBuffer, CL_TRUE, 0, n * sizeof(T),
...
TEST(filter_allpass_test, sample_float) {
  auto f = [](FilterProcessor<float> *p) {
    p->changeSampleFilter(5, vector<float>{1, 1, 1});
  };
...
TEST(filter_allpass_test, coefficient_float) {
  auto f = [](FilterProcessor<float> *p) {
    p->changeFilter(vector<float>{0, 0, 1, 0, 0});
  };
  test<float>(f, &compareFloat);
}

TEST(filter_allpass_test, sample_double) {
  auto f = [](FilterProcessor<double> *p) {
    p->changeSampleFilter(5, vector<double>{1, 1, 1});
  };
...
TEST(filter_allpass_test, coefficient_double) {
  auto f = [](FilterProcessor<double> *p) {
    p->changeFilter(vector<double>{0, 0, 1, 0, 0});
  };
  test<double>(f, &compareDouble);
}
