...
TEST(save_as_test, save_GDF_as_EDF)
{
	...
	path tmpPath = unique_path(temp_directory_path().string() + "/%%%%_%%%%_%%%%_%%%%.edf");
	...
	// The conversion introduces an error (probably in the extreme values that need to be clipped
	// in order to fit to 16 bits) so these tests have a higher tolerance. But the result looks OK
	// in Alenka.
	compareMatrixAverage(dataD.data(), gdf00.getValues().data(), channelCount, samplesRecorded, &relErr, &absErr);
	EXPECT_LT(relErr, 0.3);
	EXPECT_LT(absErr, 0.6);

//	compareMatrix(dataD.data(), gdf00.getValues().data(), channelCount, samplesRecorded, &relErr, &absErr);
