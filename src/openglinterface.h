 ...
 * This shouldn't cause any problems as OpenGL can only be used from a single thread anyway.
 *
 * @file
 */
 ...
 *
 * This object must be initialized using initializeOpenGLInterface() after an OpenGL
 * context is created and made current.
 *
 * All the public functions must be called only from within an active OpenGL context section
...
public:
	 ...
	 *
	 * At most 10 messages at a time are printed so that the log doesn't get flooded.
	 * This can happen on some platforms, where (for some reason) glGetError() never stops returning the same error over and over.
	 * In extreme cases this can lead to filling the harddrive with a huge log file.
	 *
